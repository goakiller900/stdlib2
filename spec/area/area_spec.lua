...
local A = Area
local P = require('__stdlib2-continued__/stdlib/area/position')
...
describe('Area', function ()
    ...
    describe('Area Methods', function ()
        ...
        it('.translate', function ()
            ...
            local orig = a1()
            assert.same(A(-11, -13, 11, 7), A.translate(a1, defines.direction.northeast, 1):store())
            assert.same(orig, a1:recall():translate(defines.direction.southwest, 1))
        end)
        it('.round', function ()
    ...
    describe('Area Functions', function ()
        ...
        it('.is_normalized', function ()
            assert.is_true(A(one):is_normalized())
            assert.is_false(A(1,1,-1,-1):is_normalized())
        end)
        it('.valid', function ()
        ...
        it('.is_Area', function ()
            assert.is_false(A.is_Area(one))
            assert.is_true(A(one):is_Area())
        end)
        it('.pack', function ()
